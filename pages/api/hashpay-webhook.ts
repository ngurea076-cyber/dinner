...
export default async function handler(
  ...
  console.log("🔥 HashPay webhook called!");
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      payload = req.query;
      console.log("GET payload from query:", payload);
    } else {
      ...
      if (contentType.includes("application/json")) {
        ...
        // Try to parse as JSON first, fallback to query params
        try {
          payload = req.body;
        } catch {
          payload = req.query;
    ...
    function isHashPayPayload(x: unknown): x is HashPayPayload {
      return (
        x !== null &&
        typeof x === "object" &&
        ("ResponseCode" in x || "CheckoutRequestID" in x || "TransactionID" in x)
      );
    }

    if (!isHashPayPayload(payload)) {
    ...
    if (ResponseCode !== 0 && ResponseCode !== "0") {
      console.log("Payment failed or incomplete:", ResponseDescription);
      return res
        .status(200)
    ...
    console.log("Looking for order with checkout_id:", CheckoutRequestID);
    ...
    if (!order) {
      console.error("❌ Order not found for checkout_id:", CheckoutRequestID);
      console.log("Available checkout_ids in database:");
      const allOrders = await Order.find({}, { checkout_id: 1, ticket_id: 1 });
      allOrders.forEach(o => console.log(`  ${o.checkout_id} -> ${o.ticket_id}`));
      return res.status(404).json({ success: false, error: "Order not found" });
    }

    console.log("✅ Order found and updated:", order.ticket_id);
