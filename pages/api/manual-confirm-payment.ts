...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`Order ${ticketId} manually marked as paid`);

    // Send ticket email
    try {
      if (!order.qr_code) {
        return res.status(400).json({ success: false, error: 'Order has no QR code' });
      }
        ...
        <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; border: 1px solid #e5e5e5;">
          ...
          <div style="display:flex;align-items:center;gap:20px;padding:30px;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
              ...
              <tr><td style="padding: 10px 0; border-bottom: 1px solid #f0f0f0; color: #666;">M-Pesa Ref</td><td style="padding: 10px 0; border-bottom: 1px solid #f0f0f0; text-align: right; font-family: monospace;">${transactionId || order.transaction_id || "N/A"}</td></tr>
            </table>
    ...
    console.error('Manual payment confirmation error:', message);
