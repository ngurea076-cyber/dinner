...
export default async function handler(
    ...
    try {
        ...
        for (const order of paidOrders) {
            try {
                // Check if order has QR code
                if (!order.qr_code) {
                    console.log(`Order ${order.ticket_id} has no QR code, skipping`);
                    results.push({
        ...
        `;
                ...
                if (emailError) {
                    console.error(`Email send error for ${order.ticket_id}:`, emailError);
                    results.push({
                        ticket_id: order.ticket_id,
                        status: "failed",
                        error: emailError,
                    ...
                    console.log(
                        `Ticket email sent to ${order.email} for ticket ${order.ticket_id}`,
                    );
                    results.push({
                        ticket_id: order.ticket_id,
                        status: "sent",
                ...
                console.error(`Email error for ${order.ticket_id}:`, message);
                results.push({
                    ticket_id: order.ticket_id,
                    status: "error",
                    error: message,
        ...
        return res.status(200).json({
            success: true,
            message: `Processed ${paidOrders.length} orders`,
            results,
        });
    } catch (error: unknown) {
